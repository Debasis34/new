...
function setupCategoryPage() {
    ...
    links.forEach(link => {
        link.addEventListener('click', (event) => {
            ...
            window.location.href = `button.html?category=${category}`;
        });
    });
}
...
function addCategory() {
    ...
    if (newCategoryName !== "") {
        ...
        newCategoryLink.href = `button.html?category=${newCategoryName.toLowerCase().replace(/\s+/g, '-')}`;
...
function loadSubcategories(category) {
    ...
    subcategoryItems.forEach(sub => {
        ...
        link.href = `button.html?category=${category}&subcategory=${sub.toLowerCase().replace(/\s+/g, '-')}`;
